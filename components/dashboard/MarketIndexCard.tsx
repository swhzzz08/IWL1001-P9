...
export function MarketIndexCard({ index }: { index: MarketIndex }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ height: 3, position: 'absolute', top: 0, left: 0, right: 0, borderRadius: '16px 16px 0 0',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginTop: 4 }}>
        ...
        <div style={{ width: 30, height: 30, borderRadius: 8, background: up ? '#f0fdf4' : '#fef2f2',
          display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {up ? <TrendingUp size={14} color="#16a34a" /> : <TrendingDown size={14} color="#dc2626" />}
        </div>
      </div>

      <p style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 30, letterSpacing: '-0.02em',
