        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 40, marginBottom: 40 }}>
          <div style={{ maxWidth: 280 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <div style={{ width: 32, height: 32, borderRadius: 9, background: 'linear-gradient(135deg,#0f766e,#0d9488)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <TrendingUp size={15} color="white" />
              </div>
              <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 16, color: 'var(--color-text)' }}>MarketWise</span>
            </div>
          ...
          <div style={{ display: 'flex', gap: 48 }}>
            {[
              { title: 'Platform', links: [{ href: '/', label: 'Markets' }, { href: '/learn', label: 'Learn' }, { href: '/watchlist', label: 'Watchlist' }] },
              { title: 'Learn', links: [{ href: '/learn', label: 'Stocks' }, { href: '/learn', label: 'ETFs' }, { href: '/learn', label: 'Technical Analysis' }] },
            ].map(col => (
              <div key={col.title}>
                <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-text)', marginBottom: 14, letterSpacing: '0.04em', fontFamily: 'var(--font-heading)' }}>{col.title}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {col.links.map(l => (
                    <Link key={l.label} href={l.href} style={{ fontSize: 13, color: 'var(--color-text-muted)', textDecoration: 'none' }}
