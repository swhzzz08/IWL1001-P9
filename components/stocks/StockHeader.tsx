...
export function StockHeader({ quote, isLoading }: { quote: StockQuote | null; isLoading: boolean }) {
  if (isLoading || !quote) {
    return (
      <div style={{ background: 'var(--color-surface)', border: '1.5px solid var(--color-border)', borderRadius: 16, padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <a href={`https://finance.yahoo.com/quote/${quote.symbol}`} target="_blank" rel="noopener noreferrer"
                style={{ color: 'var(--color-text-subtle)', display: 'flex' }}>
