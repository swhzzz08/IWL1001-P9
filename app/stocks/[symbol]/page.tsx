...
import { ChevronLeft, Lightbulb, BarChart2, Newspaper } from 'lucide-react'
...
export default function StockPage({ params }: { params: Promise<{ symbol: string }> }) {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* Tab switcher */}
      <div style={{ display: 'flex', gap: 2, padding: 4, background: 'var(--color-surface-2)', border: '1.5px solid var(--color-border)', borderRadius: 12, width: 'fit-content' }}>
      ...
      {tab === 'chart' && (
        <>
          <div style={{ background: 'var(--color-surface)', border: '1.5px solid var(--color-border)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ display: 'flex', gap: 20, padding: '10px 20px', borderTop: '1px solid var(--color-border)', background: 'var(--color-surface-2)', fontSize: 11, color: 'var(--color-text-muted)' }}>
